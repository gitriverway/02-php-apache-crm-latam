...
require 'controlador_cliente_enviar_correo_condiciones_renovacion_responsabilidad_civil.php';
        ...
        if (isset($_FILES["documento_" . $i])) {
            ...
            switch ($i) {
                ...
                case '9':
                    ...
                    break;
                case '10':
                    $nombreArchivo = "NUEVA VIGENCIA CONTRATO";
                    $rutaArchivo = $directorio1 . "/NUEVA-VIGENCIA-CONTRATO" . $ano1 . $mes1 . $dia1 . $hora1 . $minuto1 . $segundo1 . $cedula . "." . $_POST["extension_" . $i];
                    $archivo = $_FILES["documento_" . $i]["tmp_name"];
                    $dato = move_uploaded_file($archivo, "../../" . $rutaArchivo);
                    break;
...
for ($i = 0; $i < count($lista); $i++) {
    ...
    if ($envio_condiciones == 0 && $estado_contrato == "ACTIVO") {
        ...
        if (isset($_FILES["documento_condiciones"])) {
            ...
            $PRO = new Modelo_Proveedor();
            $nombre_proveedor = $PRO->TraerDatosProveedor($proveedor);

            $MECCR = new Envio_correo_condiciones_renovacion_responsabilidad_civil();
            $consultaEnvioCondiciones = $MECCR->realizar_envio_correo_condiciones_renovacion_responsabilidad_civil($nombre, $email, $listaCondiciones, $ruta_condiciones, $fecha_fin, $nombre_proveedor);
    ...
    $consultaMD = $MD->Modificar_Dependientes_Responsabilidad_Civil($idBayer, $idDependientes, $idContrato, $listaFamiliares, $listaVehiculos, $listaCondiciones, $contra_numero, $fecha_inicio, $fecha_fin, $estado_pago, $fechaActual, $estado_contrato, $envio_condiciones, $ruta_condiciones);
}
