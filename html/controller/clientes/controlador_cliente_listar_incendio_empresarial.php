...
class Lista_asignar_cliente
{
    ...
    function mostrar_lista_asignar_cliente()
    {
        ...
        switch ($_SESSION['S_ROL']) {
            case 'VENDEDOR':
                $consulta = $MU->listar_cliente_incendio_empresarial_asignados($idUsuario, $idCategoria);
                break;
            default:
                $consulta = $MU->listar_cliente_incendio_empresarial($idCategoria);
                break;
        }
        ...
        if (!$consulta) {
            ...
            for ($i = 0; $i < count($consulta); $i++) {
                ...
                $MCO = new Modelo_Contrato_Cliente_Empresarial();
                ...
                for ($j = 0; $j < count($consulta1); $j++) {
                    // $dependientes = $consulta1[$j]["cliente_familiares"];
                    $hogares = $consulta1[$j]["cliente_hogares"];
                }
