...
class Envio_correo_notificacion_acceso
{
    ...
    function realizar_envio_correo_notificacion_acceso($idCliente, $cedula, $estado_bayer)
    {
        ...
        $consulta = $MC->TraerDatosCliente($idCliente);

        $MU = new Modelo_Cliente();

        $consulta1 = $MU->TraerDatosUsuarioCliente($cedula);
        ...
        for ($j = 0; $j < count($consulta1); $j++) {
            $contador_envio = $consulta1[$j]["cantidad"];
        }

        if ($contador_envio == 0 && $estado_bayer == "CONTRATADO") {
