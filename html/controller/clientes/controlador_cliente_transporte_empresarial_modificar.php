...
require 'controlador_cliente_enviar_correo_condiciones_renovacion_transporte.php';
...
for ($i = 0; $i < count($lista); $i++) {
    ...
    $listaTransportes = $lista[$i]["lista_transportes"];
    ...
    if ($envio_condiciones == 0 && $estado_contrato == "ACTIVO") {
        ...
        if (isset($_FILES["documento_condiciones"])) {
            ...
            $MECCR = new Envio_correo_condiciones_renovacion_transporte();
            $consultaEnvioCondiciones = $MECCR->realizar_envio_correo_condiciones_renovacion_transporte($nombre, $email, $listaCondiciones, $ruta_condiciones, $fecha_fin);
    ...
    $consultaMD = $MD->Modificar_Dependientes_Transporte($idBayer, $idDependientes, $idContrato, $listaTransportes, $listaCondiciones, $contra_numero, $fecha_inicio, $fecha_fin, $estado_pago, $fechaActual, $estado_contrato, $envio_condiciones, $ruta_condiciones);
