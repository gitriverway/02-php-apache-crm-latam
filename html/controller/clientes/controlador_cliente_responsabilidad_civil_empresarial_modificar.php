...
require 'controlador_cliente_enviar_correo_condiciones_renovacion_responsabilidad_civil_empresarial.php';
...
$directorio3 = "view/contratos-pymes/" . $idBayer;
...
for ($i = 0; $i < count($lista); $i++) {
    ...
    if ($envio_condiciones == 0 && $estado_contrato == "ACTIVO") {
        ...
        if (isset($_FILES["documento_condiciones"])) {
            ...
            $MECCR = new Envio_correo_condiciones_renovacion_responsabilidad_civil_empresarial();
            $consultaEnvioCondiciones = $MECCR->realizar_envio_correo_condiciones_renovacion_responsabilidad_civil_empresarial($nombre, $email, $listaCondiciones, $ruta_condiciones, $fecha_fin, $nombre_proveedor);

            $envio_condiciones = 1;
        }
    }

    $MD = new Modelo_Bayer_Persona_Dependiente_Empresarial();

    $consultaMD = $MD->Modificar_Dependientes_Responsabilidad_Civil_Pymes($idBayer, $idDependientes, $idContrato, $listaFamiliares, $listaVehiculos, $listaCondiciones, $contra_numero, $fecha_inicio, $fecha_fin, $estado_pago, $fechaActual, $estado_contrato, $envio_condiciones, $ruta_condiciones);
}
