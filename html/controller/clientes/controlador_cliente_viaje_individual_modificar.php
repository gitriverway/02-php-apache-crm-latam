...
require 'controlador_cliente_enviar_correo_condiciones_renovacion_vehiculo.php';
...
for ($i = 0; $i < count($lista); $i++) {
    ...
    $listaViajes = $lista[$i]["lista_viajes"];
    ...
    if ($envio_condiciones == 0 && $estado_contrato == "ACTIVO") {
        ...
        if (isset($_FILES["documento_condiciones"])) {
            ...
            $MECCR = new Envio_correo_condiciones_renovacion_vehiculo();
            $consultaEnvioCondiciones = $MECCR->realizar_envio_correo_condiciones_renovacion_vehiculo($nombre, $email, $listaCondiciones, $ruta_condiciones, $fecha_fin, $nombre_proveedor);
    ...
    $consultaMD = $MD->Modificar_Dependientes_Viajes($idBayer, $idDependientes, $idContrato, $listaFamiliares, $listaViajes, $listaCondiciones, $contra_numero, $fecha_inicio, $fecha_fin, $estado_pago, $fechaActual, $estado_contrato, $envio_condiciones, $ruta_condiciones);
}
