...
class ControladorListaNotificacionesSeguimientoClientePymes
{
        ...
        static public function traer_lista_notificaciones_seguimiento_cliente_pymes()
        {
                ...
                if (!$consulta) {
                        ...
                        for ($i = 0; $i < count($consulta); $i++) {
                                $contar = $i + 1;
                                $lista1 .= "<a href='#' class='dropdown-item notificacionEditarCliente' idCliente = '" . $consulta[$i]["bayer_id"] . "' idCategoria = '" . $consulta[$i]["categoria_id"] . "' tipo='" . $consulta[$i]["cliente_tipo"] . "'><i class='fas fa-envelope mr-2'></i> " . $contar . "' . Modelo_Idioma::t('client_followup') . '</br><i
    ...
    <i class='far fa-bell'><strong class='d-none d-md-inline-block'>" . Modelo_Idioma::t('clients') . "
            Pymes&nbsp;</strong></i>
</a>
<div class='dropdown-menu dropdown-menu-lg dropdown-menu-left scrollable-menu'>
    ...
    <span class='dropdown-item dropdown-header'>" . Modelo_Idioma::t('list_clients') . " PYMES</span>
