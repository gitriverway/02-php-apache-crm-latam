...
class ControladorListaNotificacionesSeguimientoCliente
{
        ...
        static public function traer_lista_notificaciones_seguimiento_cliente()
        {
                ...
                if ($_SESSION['S_ROL'] == "ADMINISTRADOR" || $_SESSION['S_ROL'] == "GERENTE" || $_SESSION['S_ROL'] == "SERVICIO CLIENTE") {
                        $consulta = $MU->listar_clientes_seguimiento($fechaActual, $tipo);
                } else {
                        $consulta = $MU->listar_clientes_seguimiento_asignado($_SESSION['S_IDUSUARIO'], $fechaActual, $tipo);
                }
                ...
                if (!$consulta) {
                        ...
                        //         <i class='far fa-bell'><strong class='d-none d-sm-inline-block'><?php echo $t('clients'); ?>&nbsp;</strong></i>        
                        //         </a>
                        //         <div class='dropdown-menu dropdown-menu-lg dropdown-menu-left'>
                        //                 <span class='dropdown-item dropdown-header'>0 <?php echo $t('notifications'); ?></span>
                        //                 <span class='dropdown-item dropdown-header'><?php echo $t('list_clients'); ?></span>
                        ...
                        for ($i = 0; $i < count($consulta); $i++) {
                                $contar = $i + 1;
                                $lista1 .= "<a href='#' class='dropdown-item notificacionEditarCliente' idCliente = '" . $consulta[$i]["bayer_id"] . "' idCategoria = '" . $consulta[$i]["categoria_id"] . "' tipo='" . $consulta[$i]["cliente_tipo"] . "'><i class='fas fa-envelope mr-2'></i> " . $contar . " " . $t('client_followup') . "</br><i class='fas fa-user mr-2'></i> " . $consulta[$i]["cliente_nombre"] . " </br><i class='fas fa-calendar mr-2'></i> " . $consulta[$i]["cliente_fecha_seguimiento"] . "</a>";
                        }
                        $lista = "<a class='nav-link' data-toggle='dropdown' href='#'>
                        <span class='badge badge-danger navbar-badge'>" . $contar . "</span>
                        <i class='far fa-bell'><strong class='d-none d-md-inline-block'>" . $t('clients') . "&nbsp;</strong></i>                
                                </a>
                                <div class='dropdown-menu dropdown-menu-lg dropdown-menu-left scrollable-menu'>
                                <span class='dropdown-item dropdown-header'>" . $contar . " " . $t('notifications') . "</span>
                                <span class='dropdown-item dropdown-header'>" . $t('list_clients') . "</span>
                                        <div class='dropdown-divider'></div>
                                        " . $lista1 . "
                                        <div class='dropdown-divider'></div>
                                </div>
