    ...
    function mostrar_lista_contrato_cliente()
    {
        ...
        if (!$consulta) {
            ...
            for ($i = 0; $i < count($consulta); $i++) {
                ...
                if ($consulta[$i]["NUEVA VIGENCIA CONTRATO"] != "") {
                    $btnNuevaVigenciaContrato = "<div class='btn-group'><button style='font-size:13px;' type='button' class='btnVerDocumento btn btn-primary' ruta='" . $consulta[$i]["NUEVA VIGENCIA CONTRATO"] . "'><i class='fas fa-file-pdf'></i></button></div>";
                } else {
                    $btnNuevaVigenciaContrato = "";
                }
                ...
                switch ($consulta[$i]["cliente_estado_bayer"]) {
                    case 'CONTRATADO':
                        $estado = "<button class='btn btn-xs btn-success' estado='" . $consulta[$i]["cliente_estado_bayer"] . "'>" . $consulta[$i]["cliente_estado_bayer"] . "</button>";
                        break;
                    case 'CANCELADO':
                        $estado = "<button class='btn btn-xs btn-danger' estado='" . $consulta[$i]["cliente_estado_bayer"] . "'>" . $consulta[$i]["cliente_estado_bayer"] . "</button>";
                        break;
