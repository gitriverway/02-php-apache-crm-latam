...
class Envio_correo_notificacion_documentos_faltantes_operatorio
{
    ...
    function realizar_envio_correo_notificacion_documentos_faltantes_operatorio($idOperatorio, $idContrato)
    {
        ...
        $Ticket = 0;
        $operatorio_documentos_validados = "";
        ...
        for ($i = 0; $i < count($consulta); $i++) {
            ...
            $operatorio_documentos_validados = json_decode($consulta[$i]["operatorio_documentos_validados"], true);
        }
        ...
        foreach ($operatorio_documentos_validados as $value) {
        ...
        try {
            ...
            $mail->addReplyTo('creditohospitalario@mqpseguros.com', 'Credito Hospitalario RIVERWAYERWAY Seguros');     //Add a recipient
...
<h3 style="font-weight:100; color:#000000; padding:0px 20px;"><strong>' . $t('email_operatorio_documentos_faltantes_cliente.observation') . ': </strong> ' . $observacion_final . '</h3>
                <h3 style="font-weight:100; color:#000000; padding:0px 20px;">' . $t('email_operatorio_documentos_faltantes_cliente.upload_instructions', ['portal_url' => 'https://crm-latam.riverway.cloud']) . '
                </h3>
