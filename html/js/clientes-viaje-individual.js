...
// function lista1() {
//   $.ajax({
//     url: "/controller/clientes/controlador_cliente_listar_viaje_individual.php",
//     method: "POST",
//     cache: false,
//     contentType: false,
//     processData: false,
...
$("#tabla_cliente").on("click", ".btnBayerPersona", function () {
  var idCliente = $(this).attr("idCliente");
  window.location =
    "index.php?ruta=editar-cliente-viaje-individual&idCliente=" + idCliente;
});
...
$(".listaAdicional").on("click", "button.btnAgregarViaje", function () {
  numViaje++;

  var estado = numViaje > 1 ? "d-md-none" : "";
  var estado1 = numViaje > 1 ? "d-none" : "";
  ...
  $(this).closest(".row").find(".nuevoViaje").append(nuevoViajeHTML);
});
...
function listarDependientesContrato() {
  ...
  for (var i = 0; i < id.length; i++) {
    ...
    listaFamiliares.push({
      ...
      lista_viajes: $(lista_viajes[i]).val(),
    });
  }
...
function listarViajes() {
  var lista = $(".nuevoViaje");

  for (var j = 1; j <= lista.length; j++) {
    var listaViajes = [];

    var tipo = $(".tipoViaje" + j);
    var fechaInicio = $(".fechaInicioViaje" + j);
    var fechaFin = $(".fechaFinViaje" + j);
    var paisSalida = $(".paisSalidaViaje" + j);
    var paisDestino = $(".paisDestinoViaje" + j);
...
$(".listaAdicional").on("click", "button.quitarViaje", function () {
    ...
    success: function (respuesta) {
      ...
      if (data.length > 0) {
        if (data[0]["categoria_id"] == 7) {
          ...
          return (window.location = "clientes-viaje-individual");
        }
      } else {
        return (window.location = "clientes-viaje-individual");
      }
...
function cargar_datos_cliente_viajes() {
  var idCliente = $("#txt_idBayer").val();
  ...
  $.ajax({
    ...
    success: function (respuesta) {
      ...
      if (data.length > 0) {
        agregar_auto_lista_viajes(data);
      }
    },
  });
}
...
$(".listaAdicional").on("click", "button.btnAgregarRenovacion", function (e) {
  ...
  $(".listaAdicional").append(
    '<div class="col-md-12">' +
      ...
      '<div class="card-header">' +
      '<h3 class="card-title">Informaci&oacute;n Viajes</h3>' +
      "</div>" +
      '<div class="card-body">' +
      '<div class="row">' +
      '<div class="col-sm-12">' +
      ...
      '<div class="col-sm-12">' +
      '<div class="form-group row">' +
      '<button type="button" class="btn btn-default btnAgregarViaje" id="btnAgregarViaje' +
      num_lista_dependiente +
      '">Agregar Viaje</button>' +
      '<input type="hidden" class="listaViajes" id="listaViajes' +
      num_lista_dependiente +
      '" name="listaViajes' +
      num_lista_dependiente +
      '">' +
      '<div class="form-group row nuevoViaje" id="nuevoViaje' +
      num_lista_dependiente +
      '"></div>' +
      "</div>" +
      "</div>" +
   ...
   *******************************************************/
  $("#listaViajes" + num_lista_dependiente).val(
    $("#listaViajes" + (num_lista_dependiente - 1)).val(),
  );
  agregar_auto_viajes(num_lista_dependiente);
});
...
function agregar_auto_lista_viajes(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    $("#listaViajes" + num_lista_dependiente).val(data[i]["cliente_viajes"]);
...
function agregar_auto_viajes(idValor) {
  var lista = $("#listaViajes" + idValor).val();
...
function Modificar_Cliente() {
  ...
  $.ajax({
    url: "/controller/clientes/controlador_cliente_viaje_individual_modificar.php",
    method: "POST",
    data: datos,
    cache: false,
    contentType: false,
      ...
      if (data.length > 0) {
        if (data[0]["valor"] == 1) {
          Swal.fire(
            ...
            window.location = "clientes-viaje-individual";
          });
        } else {
          return Swal.fire(
            t("messages.warning_message", "Mensaje de Aviso"),
